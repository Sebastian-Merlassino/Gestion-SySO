...
export default function AccionesCorrectivasPage({ params }) {
  ...
  const canEdit = !profile || profile.role === 'owner' || profile.role === 'admin' || profile.permisos?.correctivas !== false;
                                ...
                                <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                                  <div className="flex items-center justify-end gap-2">
                                    ...
                                    <button
                                      ...
                                      className={`p-1.5 rounded-lg transition-all cursor-pointer inline-flex items-center ${canEdit ? 'bg-amber-50 hover:bg-amber-100 text-amber-600' : 'bg-slate-100 hover:bg-slate-200 text-slate-600'}`}
                                    >
