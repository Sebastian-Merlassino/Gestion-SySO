// src/app/[tenant-slug]/protocolos/ergonomia/components/Tabla1Modal.js
'use client';
...
export default function Tabla1Modal({ isOpen, onClose, onSelectLux }) {
  ...
  return (
    <AppInfoModal
      ...
      subtitle="TABLA 1: Intensidad Media para Diversas Clases de Tarea (Norma IRAM-AADL J 20-06)"
      icon={Info}
      maxWidth="max-w-4xl"
    ...
    >
      ...
      <div className="relative mb-3">
        ...
        <AppInput
          ...
          placeholder="Buscar por clase de tarea visual/ruido, valores o ejemplos..."
          value={searchQuery}
      ...
      <div className="border border-slate-200 rounded-xl overflow-hidden shadow-sm">
        <table className="w-full text-left text-xs text-slate-700">
          <thead className="bg-slate-100 text-slate-700 font-outfit uppercase text-[10px] tracking-wider font-extrabold border-b border-slate-200">
            <tr>
              <th className="p-3 w-[30%]">Clase de tarea</th>
              <th className="p-3 w-[20%] text-center">Valor en plano</th>
              <th className="p-3 w-[40%]">Ejemplos de tareas</th>
            ...
            {filteredItems.length === 0 ? (
              <tr>
                <td colSpan={4} className="p-6 text-center text-slate-400 font-medium">
                  No se encontraron clases de tareas con la búsqueda "{searchQuery}".
                </td>
              </tr>
            ) : (
              filteredItems.map((item, idx) => (
                <tr key={idx} className="hover:bg-blue-50/50 transition-colors">
                  ...
                  <td className="p-3 text-center">
                    ...
                    >
                      <Check className="h-3 w-3 mr-1" /> Usar {item.luxMin}
                    </AppButton>
                  </td>
                </tr>
              ))
