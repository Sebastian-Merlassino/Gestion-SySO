// src/app/[tenant-slug]/protocolos/ergonomia/[id]/pdf/page.js
'use client';
...
export default function ProtocoloErgonomiaPdfPage({ params }) {
  ...
  useEffect(() => {
    const generateAndRedirect = async () => {
      try {
        ...
        const doc = await generateErgonomyProtocolPdf(
          proto,
          tenant,
          empresas || [],
          allEstablecimientos || [],
        ...
        printPdfDocument(doc, window, `Protocolo Ergonomía - ${proto.razon_social_text || 'Reporte'}`);
      } catch (err) {
        console.error(err);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center space-y-3">
      ...
      <p className="text-xs font-semibold text-slate-600">Generando reporte PDF de Protocolo de Ergonomía...</p>
    </div>
  );
}
