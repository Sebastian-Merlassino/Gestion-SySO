// src/app/[tenant-slug]/protocolos/ruido/page.js
'use client';
...
export default function ProtocolosRuidoPage({ params }) {
  ...
  const handleSendWhatsApp = async () => {
    ...
    try {
      ...
      const doc = await generateNoiseProtocolPdf(mailTarget, tenant, empresas, allEstablecimientos, pts || [], adjs || [], isDevMode, profile);
      if (!doc) throw new Error('No se pudo generar el reporte PDF.');
      const pdfBlob = doc.output('blob');
      
      const fileId = crypto.randomUUID();
      const filePath = `${profile?.id || 'anonymous'}/ruido_${mailTarget.id}_${fileId}.pdf`;
    ...
    return (
      <div className="h-screen overflow-hidden bg-syso-bg text-slate-700 flex font-sans">
        <Sidebar
          ...
          profile={profile}
          currentSection="protocolo-ruido"
        ...
        <main className="flex-grow flex flex-col min-w-0 overflow-y-auto">
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8">
            <div className="text-center space-y-3">
              ...
              <p className="text-xs text-slate-500 font-medium">Cargando protocolo de ruido...</p>
            </div>
          </div>
        </main>
      </div>
  ...
  return (
    <div className="h-screen overflow-hidden bg-syso-bg text-slate-700 flex font-sans">
      ...
      <main className="flex-grow flex flex-col min-w-0 overflow-y-auto">
        ...
        <div className="w-full flex-grow flex flex-col min-h-0 p-0 md:py-8 md:max-w-[95%] md:mx-auto md:px-0">
          {formMode !== 'list' ? (
            <ProtocoloForm
              ...
              onClose={() => {
                ...
                setEditingId(null);
                setIsFormDirty(false);
                router.replace(`/${tenantSlug}/protocolos/ruido`);
              }}
              onSaveSuccess={() => {
                setFormMode('list');
                setEditingId(null);
          ...
          {sortedProtocolos.length === 0 ? (
            <AppEmptyState
              title="No se encontraron protocolos de ruido"
                    ...
                    return (
                      <tr 
                        ...
                        onClick={() => {
                          setEditingId(row.id);
                          setFormMode('view');
                          router.replace(`/${tenantSlug}/protocolos/ruido?id=${row.id}`);
                        }}
                              ...
                              <AppTooltip content="Editar protocolo">
                                <AppButton
                                  ...
                                  onClick={() => {
                                    setEditingId(row.id);
                                    setFormMode('edit');
                                    router.replace(`/${tenantSlug}/protocolos/ruido?id=${row.id}&action=editar`);
                                  }}
                                >
      ...
      <AppConfirmDialog
        ...
        description="¿Está seguro de que desea eliminar permanentemente este protocolo de ruido y todos sus puntos de muestreo y mediciones asociados? Esta acción no se puede deshacer."
      ...
      <AppFormNavigator
        ...
        onNavigate={(newRecord) => {
          ...
          setFormMode('view');
          router.replace(`/${tenantSlug}/protocolos/ruido?id=${newRecord.id}`);
        }}
