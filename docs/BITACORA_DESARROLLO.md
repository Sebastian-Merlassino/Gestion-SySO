...
- **Componente Reusable `AITextHelper`**: Desarrollo de un componente modular y reactivo para inputs y textareas que integra la Web Speech API para dictado de voz nativo en español, un asistente de refinamiento por IA, botón para limpiar el texto ingresado, y **robustez de excepciones para contextos sin HTTPS** (notificando claramente la falta de contexto seguro en móviles/PWA) y auto-limpieza temporal del mensaje de error tras 5 segundos.
...
- **Detección de Contexto Seguro (`isSecureContext`)**: En navegadores web, las APIs de captura de medios (como `getUserMedia` y Web Speech API) se inhabilitan automáticamente si la conexión no es `localhost` o bajo protocolo seguro `HTTPS`. Se configuró la captura del error `not-allowed` para alertar visualmente al usuario si requiere acceder mediante HTTPS seguro.
