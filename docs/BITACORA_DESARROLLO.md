...
- **Estructura y Generación de PDF del Módulo Ruido (`src/app/[tenant-slug]/protocolos/ruido/`):**
  ...
  - Se implementó la estructura acústica completa por punto de muestreo (`ProtocoloForm.js`, `pdfGenerator.js` y migración SQL `20260813000000_update_protocolo_ruido_puntos.sql`): Tiempo de exposición ($T_e$), tiempo de integración, selector de características generales del ruido (`Continuo / Intermitente` vs `De impulso o de impacto`), sub-contenedor **"SONIDO CONTINUO o INTERMITENTE"** con modalidades de carga $L_{\text{Aeq},Te}$ (dBA con evaluación dinámica del límite según la Tabla 1 para el $T_e$ cargado sin leyendas inferiores redundantes), Suma de Fracciones ($\sum C_i/T_i$ con guardado de `modo_suma_fracciones` y `fracciones` JSONB aplicados en la base de datos Supabase) y Dosis (%), o campo $L_{C\text{pico}}$ (dBC) para impulso/impacto, integrando evaluación automática de cumplimiento normativo (Res. 295/03 ANEXO V), corrección de `executeSave` para evitar `TypeError` en puntos de ruido, incorporación de `original_path` y `markers` en la tabla `protocolos_ruido_adjuntos`, actualización de la portada con la leyenda `LEY Nº 19.587 - DECRETO Nº 351/79, ANEXO V, CAPITULO 13 - ACUSTICA`, eliminación de guiones por defecto en la caja de documentación adjunta, unificación de `getPuntoCalculos` en `tablasAnexoV.js` resolviendo el `ReferenceError` al exportar PDF y rediseño completo de la tabla apaisada (Hoja 3) según el formulario oficial de Res. SRT 85/12 con súper-encabezados `RUIDO DE IMPULSO O DE IMPACTO` y `SONIDO CONTINUO o INTERMITENTE` y recuadro inferior de `Información adicional:`.
