...
### Resumen de Cambios
- **Migración de Base de Datos (`supabase/migrations/20260818006000_add_p4_medidas_to_ergonomia_puntos.sql`):**
  - Se creó la migración SQL para añadir la columna `p4_medidas JSONB NULL` a la tabla `public.protocolos_ergonomia_puntos`.
- **Integración de ANEXO I - PLANILLA 4 por Puesto Evaluado (`ProtocoloForm.js`):**
  - Se incorporó la sección **ANEXO I - PLANILLA 4: MATRIZ DE SEGUIMIENTO MEDIDAS CORRECTIVAS Y PREVENTIVAS** inmediatamente debajo de la Planilla 3 para cada puesto de trabajo evaluado.
  - **Medidas Precargadas Normativamente:** 10 filas por defecto, precargando normativamente las filas 1 a 3 (*Levantamiento y descenso*, *Transporte*, *Posturas forzadas*) y manteniendo las filas 4 a 10 editables y disponibles para medidas adicionales.
  - **Campos de Tabla:** Matriz con 8 columnas (`Nº`, `M.C.P.`, `Nombre del Puesto`, `Fecha de Evaluación`, `Nivel de riesgo`, `Fecha impl. medida Admin.`, `Fecha impl. medida Ing.`, `Fecha de Cierre`).
  - **Controles Estandarizados:** Selectores de fecha integrados con el estándar `DD/MM/AAAA` e icono interactivo de calendario `<Calendar />`, selector desplegable de Nivel de Riesgo (`Nivel 1`, `Nivel 2`, `Nivel 3`) y botón `+ Agregar Medida a Matriz`.
  - **Estandarización de Encabezados (Correlación Planilla 3 y Planilla 4):** Se unificó el diseño visual del contenedor del título de la Planilla 4 con el estándar de la Planilla 3 (`bg-blue-50/60`, borde `border-blue-100`, icono `<ShieldCheck />` azul `#468DFF`, tipografía oscura institucional, subtítulo descriptivo azul y tarjeta de puesto a la derecha).
  - **Eliminación de Barras de Desplazamiento Internas:** Se removieron las restricciones de altura máxima fija (`max-h-[360px]` / `max-h-[420px]`), las barras de desplazamiento vertical internas (`overflow-y-auto`) y los anchos rígidos de tabla (`min-w-[950px]`) en la tabla de Medidas Específicas (Planilla 3) y en la Matriz de Seguimiento (Planilla 4). Ambas tablas se expanden y adaptan fluidamente a la totalidad del ancho disponible del contenedor sin barras de desplazamiento secundarias.
  - **Remoción de Duplicación de Puesto / Área de Estudio:** Se eliminaron los botones de acción con la función `handleDuplicatePunto` en la barra de control de puesto (vistas móvil y escritorio) en `ProtocoloForm.js`.
  - **Alineación de Botón + Agregar Tarea:** Se ajustó la cabecera del contenedor de Tareas Habituales para ubicar el botón `+ Agregar Tarea` en la misma fila que el subtítulo `TAREAS HABITUALES DEL PUESTO` (alineado a la derecha mediante `flex items-center justify-between`) y con un ancho compacto `w-auto`.
  - **Texto Aclaratorio Paso 1:** Se incorporó la consigna instructiva normativamente requerida inmediatamente debajo del subtítulo de Tareas Habituales: *"Paso 1: Identificar para el puesto de trabajo, las tareas y los factores de riesgo que se presentan de forma habitual en cada una de ellas."*
  - **Unificación de Contenedor (Establecimiento y Evaluación):** Se integraron las secciones de *Datos del Establecimiento* y *Datos de la Evaluación* dentro de una única tarjeta `AppCard`, separadas por una línea divisoria sobria (`border-t border-slate-200`) para evitar la sobrecarga visual de contenedores anidados.
  - **Unificación de Contenedores de Datos Generales del Puesto:** Se eliminaron los 4 sub-contenedores individuales con bordes y sombras (`bg-white p-4 rounded-xl border shadow-sm`), unificando los 8 campos del puesto (*Área y sector*, *Puesto*, *N° trabajadores*, *Nombres*, *Procedimiento escrito*, *Capacitación*, *Manifestación temprana*, *Ubicación del síntoma*) en un único contenedor de tarjeta limpio en cuadrícula fluida.
  - **Depuración Visual de Cabecera de Evaluación:** Se removieron los elementos sobrantes tachados (*contador de puestos `(1)`*, *botón `+ Agregar Área y sector de estudio`* y *barra de subcabecera `Puesto Evaluado #1` con botón `Ver menos`*), permitiendo que el formulario de evaluación fluya directamente desde la fecha de evaluación hacia los campos del puesto sin sobrecarga visual de cabeceras redundantes.
  - **Estandarización de Subtítulo en Factores de Riesgo:** Se actualizó el subtítulo desplegable de cada factor de riesgo en la evaluación a `ANEXO I- PLANILLA 2: EVALUACION INICIAL DE FACTORES DE RIESGO — {f.label}`.
  - **Incorporación de Cabecera Planilla 1:** Se agregó el banner normativo institucional `ANEXO I- PLANILLA 1: IDENTIFICACION DE FACTORES DE RIESGO` al inicio del bloque del formulario (arriba de *Datos del Establecimiento*).
  - **Texto Aclaratorio al Pie de Factores de Riesgo:** Se incorporó la consigna instructiva al pie de la tabla de identificación de factores de riesgo: *"Si alguno de los factores de riesgo se encuentra presente, continuar con la Evaluación Inicial de Factores de Riesgo que se identifican, completando la planilla 2."*
  - **Corrección de Sintaxis de Tipos de Datos (Error 22P02):** Se convirtió la asignación de las columnas legacy de la tabla `protocolos_ergonomia_puntos` (`f_*_identificado`, `f_*_requiere_eval`) de cadenas de texto (`"si"`/`"no"`) a booleanos puros de Postgres (`true`/`false`) y `nivel_de_riesgo` a valores enteros (`1`, `2`, `3`), corrigiendo el error de sintaxis al persistir el protocolo.
  - **Persistencia:** Mapeo y guardado de `p4_medidas` en `pointsPayload` dentro de `executeSave`.
- **Alineación de Formato de Informe PDF (`pdfGenerator.js`):**
  - Se reestructuró y diseñó la generación del informe en PDF para coincidir exactamente con el documento de referencia oficial de 39 páginas de la Resolución SRT 886/15:
    - **Hoja 1 (Carátula):** Borde perimetral azul, rectángulo superior derecho con el año de evaluación, logo institucional al centro, título en azul institucional `Protocolo de Ergonomía`, subtítulo `RESOLUCIÓN 886/15` y nombre de la empresa consultora al pie.
    - **Hoja 2 (Introducción):** Membrete institucional, título `Introducción`, resumen normativo de la Res. MTESS 295/03 (Anexo I) y Res. SRT 886/15 con viñetas explicativas completas.
    - **Hoja 3 (Diagrama de Flujo):** Diagrama de flujo oficial resuelto vectorialmente para Planillas 1, 2, 3 y 4 con cuadro explicativo lateral.
    - **Hoja 4 (Datos de la Evaluación):** Secciones estructuradas para *Información del establecimiento* y *Información del puesto*.
    - **Hoja 5 (ANEXO I - PLANILLA 1):** Matriz completa en A4 Vertical con encabezado `RESOLUCIÓN S.R.T. 886/15`, consigna del Paso 1, desglose de Factores A al I, leyenda `S/E Sin Evaluar`, nota aclaratoria y bloque de 3 firmas (*Firma del empleador*, *Firma del responsable de higiene y seguridad*, *Firma del responsable de medicina laboral*).
    - **Hojas 6 a 35 (ANEXO I - PLANILLA 2):** Desglose por Factor y Tarea de la Evaluación Inicial con tablas Paso 1 y Paso 2, consignas de riesgo tolerable y bloque de 3 firmas.
    - **Hojas 36 a 38 (ANEXO I - PLANILLA 3):** Identificación de Medidas Correctivas y Preventivas (M.C.P.) por Tarea (Medidas Generales y Medidas Específicas) y bloque de 3 firmas.
    - **Hoja 39 (ANEXO I - PLANILLA 4):** Matriz de Seguimiento de Medidas Correctivas y Preventivas en 8 columnas y bloque de 3 firmas.
  - **Corrección de Errores de Exportación/Impresión:**
    1. *Eliminación de Join Inexistente:* Se corrigió la consulta en `page.js` y `[id]/pdf/page.js` eliminando el `.select('*, mediciones:protocolos_ergonomia_mediciones(*)')` que generaba HTTP 400 Bad Request en Supabase.
    2. *Carga Segura de Assets e Imágenes:* Se actualizó la ruta por defecto del logo a `/brand/logo-primary.png` y se agregaron validaciones de formato Base64 (`startsWith('data:image/')`) previas a `doc.addImage` para prevenir excepciones `wrong PNG signature`.
    3. *Manejo Defensivo de Fechas:* Se actualizó `formatDate` en `src/lib/utils.js` para procesar de forma defensiva objetos `Date`, valores numéricos y cadenas nulas sin fallar por `TypeError: dateStr.includes is not a function`.
    4. *Soporte de Firmas de Función Flexibles:* Se adaptó `generateProtocoloErgonomiaPdf` para recibir tanto firmas por objeto `{ protocolo, ... }` como argumentos posicionales `(proto, tenant, empresas, ...)` sin romper llamadas existentes.
    5. *Eliminación de Encabezados Negros en Tablas:* Se estandarizaron todas las celdas de encabezados de las Planillas 1, 2, 3 y 4 con la función auxiliar `drawHeaderBox`, redefiniendo explícitamente `setFillColor` (`COLOR_SLATE_200`) y `setDrawColor` (`COLOR_SLATE_300`) antes de cada celda para evitar que el estado de color de texto impregne el relleno de negro.
    6. *Ajuste de Desbordamiento de Texto por Caracteres Unicode:* Se implementó la sanitización automática en `drawCellText` reemplazando los caracteres Unicode no-WinAnsi (`≥` y `≤`) por `>=` y `<=`, asegurando que `splitTextToSize` calcule con precisión el ancho de línea y ajuste el texto dentro de los márgenes de cada celda.
    7. *Estandarización Unificada de Diseño PDF (Remoción de Barra Azul Superior):* Se aplicó el formato limpio de encabezados y delineados normativos:
       - **Remoción de la Barra Azul Superior (`drawProtocolTitleBar`):** Se eliminó por completo el bloque azul superior con el texto *"PROTOCOLO PARA EVALUACIÓN DE ERGONOMÍA EN EL AMBIENTE LABORAL"* de todas las páginas del informe y se reajustaron los títulos superiores a `y = 28mm`.
       - **Títulos y Subtítulos de Sección:** Encabezados en Negrita `11 pt`/`10 pt` acompañados de la línea divisoria horizontal azul de acento (`doc.setLineWidth(0.4)`, `COLOR_AZUL_PRINCIPAL`).
       - **Delineado de Tablas y Encabezados:** Celdas de encabezado en gris slate claro (`COLOR_SLATE_200`), bordes y grillas internas en gris slate (`COLOR_SLATE_300`, espesor `0.25 mm`).
       - **Bloque de Firmas Punteado (`drawTripleSignatureBlock`):** Líneas superiores punteadas estandarizadas para las firmas del Empleador, Higiene y Seguridad y Medicina Laboral.
       - **Pie de Página Dinámico (`drawFooter`):** Línea superior de acento azul, datos de contacto centrados (Empresa consultora, teléfono, correo), fecha normativamente formateada y paginación en formato `Página X de Y`.
    8. *Corrección de Error de Ejecución (`ReferenceError: factorsDefCount`):* Se definió la constante `ALL_FACTOR_KEYS` en el ámbito superior de la función `generateProtocoloErgonomiaPdf`, solucionando el `ReferenceError` al calcular la paginación global y agregando un bloque `try/catch` defensivo para la descarga de `signature_url` si la URL expira o retorna 400.
    9. *Renderizado de Imágenes de Firma y Nombres de Responsables en 3 Columnas:* Se creó la función de descarga asíncrona `fetchSignatureImage()` capaz de descargar e interpretar rutas en buckets de Supabase (`protocolos-ergonomia`, `signatures`, etc.), Base64 y URLs HTTP. Se actualizó `drawTripleSignatureBlock` para renderizar las imágenes de firma y los nombres y matrículas normativas bajo cada una de las 3 columnas (*Firma del Empleador*, *Firma del Profesional de Higiene y Seguridad*, *Firma del Profesional de Medicina del Trabajo*).
    10. *Delineado Negro en Encabezados de Tablas:* Se actualizó `drawHeaderBox` en `pdfGenerator.js` para dibujar el contorno de todas las celdas de encabezado en color negro puro (`setDrawColor(doc, COLOR_NEGRO)`), manteniendo el fondo gris claro (`COLOR_SLATE_200`), alineando perfectamente el estilo de los encabezados con el cuerpo de las tablas.
    11. *Rediseño Profesional del Diagrama de Flujo (Hoja 3):* Se reemplazó el trazado básico por un diseño vectorial de nivel ejecutivo que incluye tarjetas de proceso con bordes redondeados y barra lateral azul de acento (`drawProcessBox`), conectores de flujo con puntas de flecha sólidas rectangulares/triangulares (`drawFlowArrow`), insignias destacadas tipo pill (`drawPillBadge`) sobre las condiciones de decisión y una tarjeta de *Nota Normativa* con borde redondeado y fondo tenue en el sector inferior derecho.
    12. *Incorporación de la Tabla Oficial "Escala de Borg" en Planilla 2 (Factor 2.E):* Se implementó el renderizado dinámico de la tabla estandarizada de la Escala de Borg en las hojas de Planilla 2 correspondientes al factor `2.E: MOVIMIENTOS REPETITIVOS DE MIEMBROS SUPERIORES`, con su columna de título unificada en gris (`COLOR_SLATE_200`), 9 descripciones normativas y sus valores de 0 a 10.
    13. *Ampliación Proporcional de Firmas sin Deformación:* Se incorporó la función auxiliar `drawAspectSignature` y la lectura de dimensiones reales (`getImgDimensions`), incrementando el área máxima de renderizado a `42mm x 22mm` (y `46mm x 24mm` para Higiene y Seguridad) preservando de manera matemática el ratio original de las imágenes de firma sin estiramientos.
    14. *Incorporación del Gráfico Oficial en Alta Definición "Curva de Confort de Fanger" en Planilla 2 (Factor 2.H):* Se creó y actualizó el archivo auxiliar `fangerChartBase64.js` con el asset PNG en alta definición (HD) enviado por el usuario, implementando el renderizado dinámico del diagrama normativo de la Curva de Confort de Fanger en las hojas de Planilla 2 del factor `2.H: CONFORT TÉRMICO`, centrado y nítido sin alterar el resto de páginas del informe.
    15. *Carga Prioritaria del Logo del Administrador de Cuenta:* Se actualizó la función de carga del logo superior en `pdfGenerator.js`, priorizando las variables `logo_1_url` y `logo_url` del perfil del usuario Administrador/Owner del tenant, asegurando que los reportes de Ergonomía muestren siempre la marca institucional principal configurada.
    16. *Corrección de Alineación y Superposición de Datos en Hoja 4:* Se actualizó la función `printDataRow` asignando la posición inicial de valores a `valX = 80mm` y un ancho máximo de `115mm`, eliminando las colisiones y superposiciones entre etiquetas largas (como *Número de trabajadores en el puesto:* y *Procedimiento de trabajo escrito:*) y sus respectivos datos.
    17. *Estandarización de Portada / Carátula Específica de Ruido en Ergonomía:* Se actualizó la Hoja 1 en `pdfGenerator.js` alineándola 100% con la portada del Protocolo de Ruido (título principal en negrita de 30pt `COLOR_AZUL_PRINCIPAL` sobre 145mm, recuadro del año superior derecho, referencias normativas `LEY Nº 19.587 - DECRETO Nº 351/79` y `ANEXO I - RESOLUCIÓN SRT 886 / 2015`, logo principal del administrador centrado y consultora a `y = 246mm`).
    18. *Incorporación de la Tabla de Datos de Cabecera en Hoja 5 (Planilla 1):* Se implementó el trazado de la tabla oficial del formulario normativo de Res. SRT 886/15 directamente sobre el texto de Paso 1 en la Hoja 5 de `pdfGenerator.js` (conteniendo Razón Social, CUIT, CIIU, Dirección, Provincia, Sector, Puesto, N° de trabajadores, Capacitación, Procedimiento escrito, Nombres y Manifestación temprana), manteniendo intacto todo el contenido preexistente de la hoja.
    19. *Ajuste Fino de Textos en Portada (Hoja 1):* Se modificó la leyenda del título a `"Protocolo de Ergonomía"` y la primera referencia legal a `"Resolución 295/2003 - Anexo 1"` en `pdfGenerator.js`.
    20. *Eliminación de Hoja 4 y Rediseño de Cabecera en Planilla 1:* Se eliminó la página redundante de "Datos de la Evaluación" (antigua Hoja 4). En la Planilla 1 se eliminó el título superior de la tabla de cabecera, se ajustó la Fila 1 (Razón Social y CUIT al 50%/50%), Fila 2 (CIIU al 100%), Filas 3 a 7 con columnas de ancho equitativo (90mm / 90mm) y se incrementó la separación vertical del texto de Paso 1.
    ...
    27. *Reestructuración de Columnas en Planilla 4:* Se actualizó la tabla de Planilla 4 tanto en `ProtocoloForm.js` como en `pdfGenerator.js`, ubicando `"M.C.P."` debajo del encabezado `"Nº"`, desplazando `"Nombre del Puesto"` a la segunda columna y eliminando la tercera columna redundante.
    28. *Ajustes Finos de Estilo y Vaciado de Labels en Planilla 4:* Se unificó el color del texto `"M.C.P."` con el resto de encabezados (`text-slate-500`), se centró el texto del encabezado `"Nombre del Puesto"` y se vaciaron los textos precargados por defecto en los campos de medida (`mcp: ''`).
    29. *Paginación Dinámica de Planilla 1 en Bloques de 3 Tareas:* Se actualizó `pdfGenerator.js` dividiendo las tareas evaluadas por puesto en bloques de 3 (`taskChunks`). Si un puesto contiene 4 o más tareas (4, 5, 6, etc.), la Planilla 1 genera automáticamente páginas consecutivas (Hoja 4.1 con Tareas 1-3, Hoja 4.2 con Tareas 4-6, etc.), cumpliendo con las instrucciones normativas oficiales de Res. SRT 886/15 y actualizando en forma exacta la paginación global.
    30. *Corrección de Scope de `factorsDef`:* Se declaró `factorsDef` a nivel superior dentro de `generateProtocoloErgonomiaPdf`, solucionando el error `ReferenceError: factorsDef is not defined` durante el renderizado de la Planilla 2 y garantizando la generación fluida del documento en todas sus hojas.
...
- **Incorporación de Hojas Informativas de Acústica en el PDF de Ruido (Dec. 351/79 - Anexo V):**
  ...
  - Incluye la fórmula de dosis / exposición combinada $\frac{C_1}{T_1} + \frac{C_2}{T_2} + \dots \le 1$, la **Tabla 1 de Valores Límite Umbral para Ruido Continuo o Intermitente** (con las 21 duraciones de 24h a 0.11s), la **Tabla de Valores Límite para Ultrasonido** (10 kHz a 100 kHz) y sus 3 aclaraciones/notas al pie normativas (`*`, `**`, indicación de aceleración en $g \cdot v.c.m.$).
  - Se igualó la función `printParagraph` y su cálculo de interlineado/espaciado entre párrafos (`blockH = (lines.length * 4.2) + 4.5`, tamaño de fuente 9 pt y alineación justificada) exactamente al estándar visual utilizado en el PDF del Protocolo de Iluminación.
  - Se corrigió el delineado (borde/stroke) de la fila de títulos/encabezados de las tablas normativas (`Tabla 1: Valores Límite Umbral para Ruido` y `Valores Límite para Ultrasonido`) configurando `setDrawColor(doc, COLOR_SLATE_300)` de forma explícita antes de sus celdas para que coincida exactamente con las filas de datos.
...
- **Estructura y Generación de PDF del Módulo Ruido (`src/app/[tenant-slug]/protocolos/ruido/`):**
  ...
  - Se implementó la estructura acústica completa por punto de muestreo (`ProtocoloForm.js`, `pdfGenerator.js`, migración SQL `20260813000000_update_protocolo_ruido_puntos.sql` y `20260814000000_add_informacion_adicional_protocolo_ruido.sql`): Tiempo de exposición ($T_e$), tiempo de integración, selector de características generales del ruido (`Continuo / Intermitente` vs `De impulso o de impacto`), sub-contenedor **"SONIDO CONTINUO o INTERMITENTE"** con modalidades de carga $L_{\text{Aeq},Te}$ (dBA con evaluación dinámica del límite según la Tabla 1 para el $T_e$ cargado sin leyendas inferiores redundantes), Suma de Fracciones ($\sum C_i/T_i$ con guardado de `modo_suma_fracciones` y `fracciones` JSONB aplicados en la base de datos Supabase) y Dosis (%), o campo $L_{C\text{pico}}$ (dBC) para impulso/impacto, integrando evaluación automática de cumplimiento normativo (Res. 295/03 ANEXO V), eliminación completa de las planillas del método de la cuadrícula heredadas del protocolo de iluminación, ajuste estricto de las páginas de planos al número exacto de imágenes de croquis adjuntas, incorporación de la tarjeta **"Información adicional"** con área de texto expandible y dictado por voz/refinamiento por IA (`SySO-AI-Voice-Helper`) ubicada justo debajo de *Documentación que se Adjuntará a la Medición*, actualización de la portada con la leyenda `LEY Nº 19.587 - DECRETO Nº 351/79, ANEXO V, CAPITULO 13 - ACUSTICA`, eliminación de guiones por defecto en la caja de documentación adjunta y rediseño completo de la tabla apaisada (Hoja 3) según el formulario oficial de Res. SRT 85/12.
  ...
  - Se implementó la doble modalidad para la opción Suma de Fracciones ($\sum C_i/T_i$): selector entre **Resultado Directo** y **Desglose de Fracciones ($C_1/T_1 + C_2/T_2 + ... + C_n/T_n$)** con agregado/eliminación dinámica de filas, cálculo en tiempo real de $C_i/T_i$, persistencia asegurada del contenedor inicial y actualización automática del total acumulado en el punto.
  - Se creó el catálogo normativo `tablasAnexoV.js` y el componente de la ventana emergente `Tabla1RuidoModal.js`, agregando el botón **"Ver Tabla 1"** junto al título del desglose de fracciones y junto a **Características generales del ruido a medir** para consultar la **Tabla 1 — Valores límite para ruido** con columna titulada **"Duración por día"** (24 horas a 80 dBA hasta 0,11 segundos a 139 dBA).
...
- **Modulo de Protocolo de Iluminación**:
  ...
  - **Indicadores de Campos Obligatorios**: Se agregaron props `required` o `required={estado === 'completado'}` a las etiquetas `AppLabel` de los campos obligatorios correspondientes (Razón Social, Establecimiento, Instrumento, Fecha de Medición, y condicionalmente Fecha de Calibración, Metodología, Sector y Valor Requerido). Esto renderiza de forma unificada el asterisco azul (`*`) indicativo de campo obligatorio en el formulario.
  - **Corrección de Columnas de Firma Profesional en BD**: Se detectó y solucionó el error `PGRST204` al guardar un protocolo provocado por la falta de las columnas de firma del profesional en la base de datos real. Se agregaron mediante migración SQL las columnas `firma_profesional`, `profesional_nombre`, `profesional_matricula` y `firma_tipo` a la tabla `protocolos_iluminacion` y se recargó el caché de PostgREST, permitiendo guardar el formulario sin errores.
  - **Navegación por Fila y Filtro de Acciones (Ojo)**: En la vista de listado de protocolos (`src/app/[tenant-slug]/protocolos/iluminacion/page.js`), se agregó un manejador `onClick` en los elementos `<tr>` de la tabla para abrir la vista de detalles del protocolo correspondiente al hacer clic en cualquier parte de la fila. Además, se limitó la visualización del botón de vista rápida (ojo 👁️) únicamente para usuarios con rol de `cliente`, retirándolo del panel de acciones de administradores/profesionales de forma consistente.
  - **Estandarización del Modo Vista (Lectura)**: En `ProtocoloForm.js` se implementó el comportamiento estándar para la visualización de detalles en modo lectura (`mode === 'view'`). Se ocultó el botón "Eliminar" del pie del formulario (el cual ahora se muestra únicamente en modo de edición) y se incorporó el botón azul "Editar" en su lugar para usuarios autorizados (`canEditar`). Al pulsarlo, redirige al usuario a la ruta de edición del protocolo. Además, al estar en modo lectura, los botones de eliminación de imágenes individuales en `ImageUploadZone` quedan deshabilitados correctamente por diseño.
  - **Corrección de Insert en Supabase**: Se solucionó el error de tipo de JavaScript `TypeError: s.O.from(...).select(...).insert is not a function` al guardar un protocolo con imágenes adjuntas. Se corrigió el encadenamiento del builder de Supabase en `ProtocoloForm.js` a `.insert(pointsPayload).select()` (en lugar de `.select().insert(pointsPayload)`).
  - **Estandarización de Compartir por Correo y WhatsApp**: Se reestructuró el modal de envío en el listado de protocolos de iluminación para implementar el estándar unificado de la aplicación. Ahora cuenta con dos pestañas interactivas (Correo Electrónico y WhatsApp) que obtienen los datos de contacto correspondientes de la Razón Social y permiten compartir el enlace firmado del PDF.
  - **Estandarización de Botones de Acciones**: Se reemplazaron todos los botones de la columna de acciones en la tabla de listado con el componente unificado `AppButton`, alineando el orden, los iconos y colores (`bg-slate-100` para descargar/imprimir, `bg-[#EFF6FF]` para ver detalles, `bg-blue-50` para enviar, `bg-indigo-50` para duplicar, `bg-amber-50` para editar y `bg-red-50` para eliminar), igualando al resto de los módulos de la aplicación.
