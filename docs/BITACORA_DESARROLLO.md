...
- **Tooltips Interactivos en Gráfico (Dashboard)**: Se reemplazaron los tooltips nativos del navegador (`title`) por una ventana emergente premium hecha con HTML/Tailwind (`hidden group-hover:flex`). Estos tooltips se visualizan de manera instantánea al pasar el mouse por encima de cualquiera de las 14 columnas, mostrando la descripción de la métrica activa, el valor numérico con formato Outfit destacado, y la aclaración de la unidad de medida según el índice seleccionado (Incidencia, Mortalidad, Pérdida o DMB).
