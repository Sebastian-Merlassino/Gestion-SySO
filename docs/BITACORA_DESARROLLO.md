...
- **Pie de Página Dinámico en PDF de Constancia (`visitas/page.js`)**: Se refactorizó la función de generación del PDF para que el pie de página de la constancia (`drawHeaderAndFooter`) lea dinámicamente el nombre de la empresa/consultora (`tenant.name`), el teléfono (`profile.phone`) y el correo (`profile.email`) del usuario que actualmente tiene iniciada la sesión, en lugar de pintar datos de soporte estáticos.
