...
- **Unificación de Pies de Página**: Se modificaron las secciones de Dashboard (siniestralidad), Constancias de Visita y Avisos de Riesgo para que el pie de página de sus PDFs sea idéntico y muestre únicamente el nombre de la consultora, el teléfono y el correo del administrador del tenant (obtenidos dinámicamente de `profiles` donde `role = 'admin'`), junto a la numeración de páginas.
