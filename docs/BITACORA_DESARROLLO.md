...
- **Estructura y Generación de PDF del Módulo Ruido (`src/app/[tenant-slug]/protocolos/ruido/`):**
  ...
  - Se implementó la estructura acústica completa por punto de muestreo (`ProtocoloForm.js`, `pdfGenerator.js` y migración SQL `20260813000000_update_protocolo_ruido_puntos.sql`): Tiempo de exposición ($T_e$), tiempo de integración, selector de características generales del ruido (`Continuo / Intermitente` vs `De impulso o de impacto`), sub-contenedor **"SONIDO CONTINUO o INTERMITENTE"** con modalidades de carga $L_{\text{Aeq},Te}$ (dBA con evaluación dinámica del límite según la Tabla 1 para el $T_e$ cargado), Suma de Fracciones ($\sum C_i/T_i$) y Dosis (%), o campo $L_{C\text{pico}}$ (dBC) para impulso/impacto, integrando evaluación automática de cumplimiento normativo (Res. 295/03 ANEXO V) y actualización de las columnas en la tabla apaisada del PDF.
